...
import { Artista } from '@/src/models.tsx'; // Importando seu tipo Artista
import { deleteArtist } from '@/actions/artista.tsx'; // <-- 1. Importe a nova ação
import { ArtistFormModal } from './modal.tsx'; // <-- Importa o novo modal
...
export function ArtistsTable({ artists, totalCount }: ArtistsTableProps) {
  ...
  const handleOpenEditModal = (user: Artista) => {
    setEditingArtist(user); // Define o usuário para edição
    setIsFormModalOpen(true);
  };
  ...
  const handleOpenDeleteDialog = (user: Artista) => {
    setArtistToDelete(user);
    setDialogOpen(true);
  };
  ...
  return (
    <Box>
      ...
      <Paper sx={{ width: '100%', mb: 2, borderRadius: 2, boxShadow: 1 }}>
        <TableContainer>
          <Table stickyHeader aria-label="Tabela de Artistas">
            <TableHead>
              <TableRow>
            ...
            <TableBody>
              ...
              {artists.length === 0 && (
                <TableRow><TableCell colSpan={5} align="center">Nenhum artista encontrado.</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </TableContainer>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            ...
            <strong>{` ${artistToDelete?.nome || ''}`}</strong>? Esta ação não pode ser desfeita.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
