...
import { Autor } from '@/src/models.tsx'; // Importando seu tipo Autor
import { deleteAutor } from '@/actions/autor.tsx'; // <-- 1. Importe a nova ação
import { AutorFormModal } from './modal.tsx'; // <-- Importa o novo modal
...
export function AutoresTable({ autores, totalCount }: AutoresTableProps) {
  ...
  const handleOpenCreateModal = () => {
    setEditingAutor(null); // Limpa o usuário em edição (modo de criação)
    setIsFormModalOpen(true);
  };

  const handleOpenEditModal = (user: Autor) => {
    setEditingAutor(user); // Define o usuário para edição
    setIsFormModalOpen(true);
  };
  ...
  const handleOpenDeleteDialog = (user: Autor) => {
    setAutorToDelete(user);
    setDialogOpen(true);
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    const result = await deleteAutor(autorToDelete.id);
  ...
  return (
    <Box>
      ...
      <Paper sx={{ width: '100%', mb: 2, borderRadius: 2, boxShadow: 1 }}>
        <TableContainer>
          <Table stickyHeader aria-label="Tabela de Autors">
            <TableHead>
              <TableRow>
            ...
            <TableBody>
              ...
              {autores.length === 0 && (
                <TableRow><TableCell colSpan={5} align="center">Nenhum autor encontrado.</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </TableContainer>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            ...
            <strong>{` ${autorToDelete?.nome || ''}`}</strong>? Esta ação não pode ser desfeita.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
